...
public class ClientServerTest {
  ...
  public void testFireNForget10() {
    long outputCount = Flux.range(1, 10)
        .flatMap(i -> setup.getRSocket().fireAndForget(DefaultPayload.create("hello", "metadata")))
        .doOnError(Throwable::printStackTrace).count().block();
  ...
  public void testRequestResponse1() {
    long outputCount = Flux
        .range(1, 1).flatMap(i -> setup.getRSocket()
            .requestResponse(DefaultPayload.create("hello", "metadata")).map(Payload::getDataUtf8))
        .doOnError(Throwable::printStackTrace).count().block();
  ...
  public void testRequestResponse10() {
    long outputCount = Flux
        .range(1, 10).flatMap(i -> setup.getRSocket()
            .requestResponse(DefaultPayload.create("hello", "metadata")).map(Payload::getDataUtf8))
        .doOnError(Throwable::printStackTrace).count().block();
  ...
  public void testRequestResponse100() {
    long outputCount = Flux
        .range(1, 100).flatMap(i -> setup.getRSocket()
            .requestResponse(DefaultPayload.create("hello", "metadata")).map(Payload::getDataUtf8))
        .doOnError(Throwable::printStackTrace).count().block();
  ...
  public void testRequestResponse10_000() {
    long outputCount = Flux
        .range(1, 10_000).flatMap(i -> setup.getRSocket()
            .requestResponse(DefaultPayload.create("hello", "metadata")).map(Payload::getDataUtf8))
        .doOnError(Throwable::printStackTrace).count().block();
  ...
  public void testRequestStream() {
    Flux<Payload> publisher = setup.getRSocket()
        .requestStream(DefaultPayload.create("hello", "metadata"));
