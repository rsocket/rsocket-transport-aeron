...
public class AeronClientTransport implements ClientTransport {
  ...
  public AeronClientTransport(WorkQueueProcessor<Runnable> workQueueProcessor, Aeron aeron,
  ...
  public Mono<DuplexConnection> connect() {
    ...
    Subscription clientSubscription = aeron.addSubscription(aeronURL, streamId,
        image -> onImageAvailable.onComplete(), image -> {
        });
    return Mono.defer(() -> {
      logger.info("creating new connection with connection id {} and aeron stream id {} to url {}",
          connectionId, streamId, aeronURL);

      MonoProcessor<DuplexConnection> notify = MonoProcessor.create();

      ConcurrentPublication serverManagementPublication = aeron.addPublication(aeronURL,
          Constants.SERVER_MANAGEMENT_STREAM_ID);

      Subscription clientManagementSubscription = aeron.addSubscription(aeronURL,
          Constants.CLIENT_MANAGEMENT_STREAM_ID);

      AeronSubscriptionFlux
          .create("clientManagementStream", workQueueProcessor, clientManagementSubscription,
              allocator)
          .filter(byteBuf -> AeronPayloadFlyweight.frameType(byteBuf) == FrameType.SETUP_COMPLETE
          ...
          .flatMap(byteBuf -> {
            ...
            return Mono.<DuplexConnection>create(sink -> {
              ...
              logger.info(
                  ...
                  connectionId, serverStreamId, serverAeronUrl);

              AeronDuplexConnection connection = new AeronDuplexConnection("client",
                  workQueueProcessor, aeron.addPublication(serverAeronUrl, serverStreamId),
                  clientSubscription, allocator);

              sink.success(connection);
            });
          }).doFinally(signalType -> clientManagementSubscription.close()).subscribe(notify);

      ByteBuf encode = SetupFlyweight.encode(allocator, connectionId, streamId, aeronURL);

      return Mono.just(encode)
          .transform(Operators
              .lift((s, c) -> AeronPublicationSubscriber.create("serverManagementPublication",
                  workQueueProcessor, c, serverManagementPublication)))
          .doFinally(signalType -> serverManagementPublication.close()).then(onImageAvailable)
          .then(notify).doOnSuccess(duplexConnection -> {
            logger.info(
                "successfully created new connection with connection id {} and aeron stream id {} to url {}",
                connectionId, streamId, aeronURL);
          }).timeout(Duration.ofSeconds(1000)).onErrorMap(throwable -> {
            clientManagementSubscription.close();
            logger.error("error trying to make a connection to " + aeronURL, throwable);
            if (throwable instanceof TimeoutException) {
              return new TimeoutException("timed out waiting for connection to server " + aeronURL);
            } else {
              return throwable;
            }
          });
